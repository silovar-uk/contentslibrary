...
const appHostname = required('APP_HOSTNAME').toLowerCase();
...
if (!/^(?=.{4,253}$)(?!-)[a-z0-9.-]+(?<!-)$/.test(appHostname) || appHostname.includes('/') || !appHostname.includes('.')) throw new Error('APP_HOSTNAME must be a hostname such as library.example.com.');
...
const config = {
  ...
  workers_dev: false,
  routes: [{ pattern: appHostname, custom_domain: true }],
  assets: {
    directory: './public',
    binding: 'ASSETS',
...
console.log(`Production config generated for https://${appHostname} (owner bootstrap: ${allowOwnerBootstrap}).`);
