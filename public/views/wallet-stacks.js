...
function coverMarkup(work, index) {
  ...
  return `<button type="button" class="wallet-stack-book ${hasCover ? "has-cover" : "no-cover"}" data-open-work="${esc(work.id)}" style="--stack-index:${index}" aria-label="${esc(work.title)}を開く">${face}</button>`;
}
...
function openStackInLibrary(key) {
  const value = key === "top" ? "top" : "";
  const status = key === "owned" ? "owned_unread" : key === "want" ? "want" : "";
  document.querySelector(".mobile-nav [data-mobile-view='library']")?.click();
