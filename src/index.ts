...
import { getLibrarySnapshot } from "./routes/library-snapshot";
...
import { listDecisions, syncDecisions } from "./routes/decisions";
import {
  commitImportBatch,
  createImportBatch,
  deleteImportBatch,
...
async function handleApi(request: Request, env: Env, auth: AuthContext): Promise<Response> {
  ...
  if (request.method === "GET" && path === "/api/decisions") return listDecisions(env, auth);
  if (request.method === "POST" && path === "/api/decisions/sync") return syncDecisions(request, env, auth);
  ...
  if (m && request.method === "POST") return importWorkFactsV21(request, env, auth, decodeURIComponent(m[1]!));
